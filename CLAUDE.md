...
**Practical consequence: sitewide changes must be hand-applied to every HTML file.** There are currently 17 HTML pages: `index.html`, 9 under `guides/` (`guides/index.html` + 8 articles — ChatGPT, Claude, Suno, iCloud+, CapCut, the "как оплатить зарубежные сервисы" pillar guide, App Store region change, Apple balance write-off), and 7 under `legal/` (`legal/index.html` + 6 legal docs). Anything "global" — the Yandex Metrika snippet, CTA click-tracking script, JSON-LD boilerplate, OG/Twitter meta, design tokens — is duplicated verbatim in each file's `<head>`/`<body>`. When editing one of these, grep across all 17 files rather than assuming a single source of truth. Small scripted edits (Python/sed over the file list) are more reliable here than manual per-file editing.
...
**Bot in-app navigation path for Apple ID top-ups is `Каталог → Пополняй AppStore`** — the subscription guides (ChatGPT, Claude, Suno, iCloud+, CapCut, the pillar guide) describe this step in a `step-desc` div and in the matching `HowTo` JSON-LD `text` field; both copies must be kept in sync with the bot's actual menu wording if it changes again.

**Adding a new subscription guide**: copy the most recently updated service guide (e.g. `guides/capcut/index.html`) — it carries the full current template (consent-gated Metrika, JSON-LD graph, cookie banner, CTA tracking, note block with the region-change button, `steps-title` subheading). Then register the new page in five places: `guides/index.html` (card + `ItemList` JSON-LD), the homepage guides carousel in `index.html`, `sitemap.xml`, `llms.txt`, and this file's page count. Service app icons live in `icons/` — if one is missing, fetch the App Store artwork via `https://itunes.apple.com/lookup?id=<appId>` (`artworkUrl100`) and save as PNG; dark-on-white icons get inverted to white-on-black to fit the theme (see `icons/capcut.png`).
...
- `guides/` mixes two article shapes: service-specific how-tos (ChatGPT, Claude, Suno, iCloud+, CapCut, region change, balance write-off) and a broader pillar/hub page (`kak-oplatit-zarubezhnye-servisy`) that explains the general method once and links out to the specific guides — use the same pattern for future broad-intent guides rather than duplicating the general explanation into every article.
