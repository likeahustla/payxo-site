...
**Practical consequence: sitewide changes must be hand-applied to every HTML file.** There are currently 21 HTML pages (20 listed below plus the unlisted partner page `hugo/index.html`, see "Partner offer page" below): `index.html`, 12 under `guides/` (`guides/index.html` + 11 articles — ChatGPT, Claude, Suno, iCloud+, CapCut, the CapCut-via-Turkey price guide, FaceApp, Higgsfield (virtual-VISA-card flow with embedded Telegram mockups), the "как оплатить зарубежные сервисы" pillar guide, App Store region change, Apple balance write-off), and 7 under `legal/` (`legal/index.html` + 6 legal docs). Anything "global" — the Yandex Metrika snippet, CTA click-tracking script, JSON-LD boilerplate, OG/Twitter meta, design tokens — is duplicated verbatim in each file's `<head>`/`<body>`. When editing one of these, grep across all 20 files rather than assuming a single source of truth. Small scripted edits (Python/sed over the file list) are more reliable here than manual per-file editing.
...
**Brand visual system on guide pages** (pillar guide, Higgsfield): `.facts` grid of `.fact-card`s with `.fact-icon` chips (emoji or lime `✦`; numbered chips colored via `a-lime`/`a-sky`/`a-lav`), decorative background shapes (`.deco-x` ✕ glyph, `.deco-o` ring, guarded by `main > :not(.deco){position:relative;z-index:1}`), and a lime `h1 .accent` span. Reuse these classes for new richly-styled guides instead of inventing new ones.

**Higgsfield guide anatomy** (`guides/higgsfield/`): steps are Telegram phone-frame mockups — `.mstep` two-column grid (sticky `.mnote` copy on the left, fixed 360px `.phone` pinned right, single column under 820px). Every `.mnote` MUST close before its `.phone` sibling or the grid collapses (this broke once). Service icons render via CSS classes `.cicon.ic-<name>` whose background is a base64 SVG data-URI declared once in `<style>` (repeats are free); a few icons are inline base64 PNG `<img class="cicon">`. Static icon sources (frames of the payxo_icons custom-emoji pack) live OUTSIDE the repo in `~/Documents/PayXO/payxo-icons-export/static/` (SVG + PNG, README inside). Mockup captions must be written as instructions to the reader («Нажмите…»), never as design commentary («Покупатель открывает…», «Сервисы — сеткой по два») — the mockup artifact's own notes are specs, not guide copy.

**Partner offer page** (`hugo/index.html`, URL `/hugo/`): a personal commercial offer for the AI creator Hugo (Hugency Academy) — lifetime 5% referral share, income calculator (inline JS, defaults 1 000 students × 7 500 ₽/month), trust cards, single CTA «Открыть бот PayXO». Uses the standard site header/footer/cookie banner/Metrika/CTA tracking (`data-analytics="hugo"`). It is deliberately `noindex, nofollow` and is NOT listed in `sitemap.xml`, `llms.txt`, `guides/` or the homepage — reachable only by direct link. The repo is public, so the page is technically readable by anyone who knows the URL. Copy conventions: brand is always written «PayXO»; the bot delivers «реквизиты виртуальной карты VISA», never «доступ»; income figures are labeled as illustration, not a guarantee. The «Что можно оплатить» grid uses service icons from `icons/ai/` (static frames of the payxo_icons pack, copied from `~/Documents/PayXO/payxo-icons-export/static/`; 12 services have one) — services without an icon file render a lime monogram tile, so drop a new file into `icons/ai/` and swap the `.mono` span for an `<img>` when an icon appears. The service list mirrors the bot's payment-cards section (32 services) plus Seedance. The calculator numbers (`#calc` JS + static scenario cards) are duplicated in the HTML — change both.
