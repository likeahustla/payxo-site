...
**Practical consequence: sitewide changes must be hand-applied to every HTML file.** There are currently 15 HTML pages (`index.html`, 6 under `guides/`, 7 under `legal/` + `legal/index.html`, `guides/index.html`). Anything "global" — the Yandex Metrika snippet, CTA click-tracking script, JSON-LD boilerplate, OG/Twitter meta, design tokens — is duplicated verbatim in each file's `<head>`/`<body>`. When editing one of these, grep across all 15 files rather than assuming a single source of truth. Small scripted edits (Python/sed over the file list) are more reliable here than manual per-file editing.
